...
const mostLikes = (blogs) =>
{
    ...
    blogs.forEach(element => {
        ...
        if(authorName === undefined)
        {
            ...
            blogCount.find(blog => blog.author === element.author).likes += element.likes
            
    });

    const max =  Math.max(...blogCount.map(blog => blog.likes));
    return blogCount.find(blog => blog.likes === max)
